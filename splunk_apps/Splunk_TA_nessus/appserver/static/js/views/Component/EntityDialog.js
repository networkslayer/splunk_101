    ...
    return Backbone.View.extend({
        ...
        render: function () {
            ...
            _.each(entity, function (e) {
                ...
                if (e.field === 'server') {
                    this._loadServers(controlWrapper);
                }
            ...
            if (this.component.model === Server) {
        ...
        _loadServers: function (controlWrapper) {
            this.servers = new Servers([], {
                appData: {app: appData.get("app"), owner: appData.get("owner")},
                targetApp: this.addonName,
                targetOwner: "nobody"
            });
            var servers_defered = this.servers.fetch();
            servers_defered.done(function () {
                var dic = _.map(this.servers.models, function (server) {
                    return {
                        label: server.entry.attributes.name,
                        value: server.entry.attributes.name
                    };
                });
