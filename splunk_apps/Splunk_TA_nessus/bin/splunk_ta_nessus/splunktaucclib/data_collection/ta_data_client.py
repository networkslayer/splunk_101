...
class TaDataClient(object):
    def __init__(self, all_conf_contents, meta_config, task_config, ckpt=None,
                 checkpoint_mgr=None):
...
def create_data_collector(dataloader, tconfig, meta_configs, task_config,
                          data_client_cls,
                          checkpoint_cls=None):
...
def client_adatper(job_func):
    class TaDataClientAdapter(TaDataClient):
        def __init__(self, all_conf_contents, meta_config, task_config, ckpt,
                     chp_mgr):
            super(TaDataClientAdapter, self).__init__(all_conf_contents,
                                                      meta_config,
                                                      task_config,
                                                      ckpt, chp_mgr)
